...
They live here rather than beside one package because **both converters run against them**: the browser one under `packages/bim-ifc-converter` and the Python one under `python/`. Two implementations of one job are only comparable if they are asked the same questions, and a copy of a fixture is a question that drifts. They are small enough to read, and they are committed as plain files rather than through Git LFS so the tests run on a fresh clone with nothing fetched.
