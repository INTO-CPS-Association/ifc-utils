...
- A model the catalogue does not name shows its file name with its separators read as spaces, `Building_1912_AK_v4` as "Building 1912 AK v4", through `readableName`. The case is left alone, because in these names it carries meaning. The catalogue itself is generated from the IFC files by `ifc-to-catalogue` in `ifc-converter`, and a person names what a file does not in `catalogue.overrides.json`.
