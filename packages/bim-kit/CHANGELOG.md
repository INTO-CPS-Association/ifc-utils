...
- The model picker sits on the page instead of filling the row inside a padded panel of its own, which made it read as a different kind of control. It is wide enough (30rem) for the longest name in the DTaaS library to show in full.
...
- A model stored from the browser now reads as Converted. The listing read again after the save could come from the browser cache, since Jupyter sends it without Cache-Control, so the new `.glb` was missing and the chip stayed at From IFC. The listing is requested with `cache: 'no-store'`.
