...
# instead of left out, so a person can search for what is still open.
TODO_ROLE = "TODO"
...
# them in a property set of their own naming. Any set is searched instead of
...
def topic_for(sensor, model, prefix=None):
    ...
    thing it describes instead of by whoever wrote this converter.
    """
...
def proposal_of(sensors):
    """Whether every sensor was proposed instead of surveyed, and on what basis.
...
def as_yaml(manifest):
    ...
    header = [
        ...
        "ifc_explorer.to_manifest, so regenerate instead of edit by hand when",
        "the model changes.",
        "",
    ...
    # provenance, passed through instead of listed again here. Listing it
    ...
    document = manifest_module.document(
        ...
        [
            manifest_module.binding(
                ...
                # so. Recorded as a field instead of a comment, so a reader
                # does not have to parse comments to find it.
                mounted_on=b["host"],
                # Not derivable from IFC, and named instead of left silent.
                role=TODO_ROLE,
            )
...
def main(argv=None):
    ...
    if not manifest["bindings"]:
        ...
        # an empty manifest looks like a bug instead of a fact about the file.
